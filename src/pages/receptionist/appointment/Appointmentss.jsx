...
import {
  Plus, Edit, Calendar, X, Search, UserCheck, Phone, Mail, Clock, Check, AlertTriangle
} from 'lucide-react'
...
export default function Appointments() {
  ...
  const [formData, setFormData] = useState({
    ...
    doctorName: '', appointmentDate: '', appointmentTime: '', appointmentType: 'consultation',
  ...
  const handleCreateAppointment = () => {
    setFormData({
      patientName: '', patientPhone: '', patientEmail: '', patientAge: '', patientGender: '',
      doctorName: '', appointmentDate: getMinDate(), appointmentTime: '', appointmentType: 'consultation',
      notes: '', status: 'scheduled', symptoms: '', medicalHistory: '', medications: '',
      vitalSigns: { bloodPressure: '', heartRate: '', temperature: '', weight: '' }
    })
  ...
  const validateForm = () => {
    ...
    if (!formData.doctorName.trim()) return toast.error('Please select a doctor') && false
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (showEditModal) {
        await updateDoc(doc(db, 'appointments', selectedAppointment.id), { ...formData, updatedAt: new Date().toISOString() })
        toast.success('Appointment updated!')
        setShowEditModal(false)
      } else {
        await addDoc(collection(db, 'appointments'), { ...formData, createdAt: new Date().toISOString(), createdBy: currentUser?.uid || 'receptionist', status: 'scheduled' })
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 focus:border-transparent outline-none"
              />
            </div>
            <select
              value={filterStatus}
              onChange={(e) => setFilterStatus(e.target.value)}
              className="px-4 py-2 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 outline-none"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-700 text-white rounded-lg transition-colors shadow"
          >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-700 dark:text-slate-300">{appointment.doctorName}</span>
                        </div>
                        <div className="flex items-center gap-2">
      ...
      {(showCreateModal || showEditModal) && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-2xl p-6 w-full max-w-2xl max-h- overflow-y-auto shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.patientName} onChange={(e) => setFormData({ ...formData, patientName: e.target.value })}
                    className="w-full px-3 py-2 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 outline-none" required />
                </div>
                <div>
                  ...
                  <select value={formData.doctorName} onChange={(e) => setFormData({ ...formData, doctorName: e.target.value })}
                    ...
                    {doctors.map((doctor) => <option key={doctor.id} value={doctor.fullName || doctor.name}>Dr. {doctor.fullName || doctor.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Appointment Date</label>
                  <input type="date" value={formData.appointmentDate} onChange={(e) => setFormData({ ...formData, appointmentDate: e.target.value })} min={getMinDate()} max={getMaxDate()}
                    className="w-full px-3 py-2 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 outline-none [color-scheme:light] dark:[color-scheme:dark]" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Appointment Time</label>
                  <select value={formData.appointmentTime} onChange={(e) => setFormData({ ...formData, appointmentTime: e.target.value })}
                    className="w-full px-3 py-2 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-cyan-400 outline-none" required>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button type="button" onClick={() => { setShowCreateModal(false); setShowEditModal(false) }}
                  className="px-4 py-2 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
                  Cancel
                </button>
                <button type="submit" disabled={loading}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-700 text-white rounded-lg disabled:opacity-50">
